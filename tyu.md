...
## Request flow with mTLS enabled (production)

**Production only.** Kong Data Plane (cloud) talks **directly** to your backend over HTTPS with a client certificate. There is **no ngrok** in production—ngrok is only for local testing so Konnect can reach your laptop.

When mTLS is on, Kong presents a **client certificate** when calling your backend; your mTLS proxy (or TLS termination layer) **verifies** that cert. Only Kong can complete the TLS handshake and reach the app.
...
flowchart LR
    ...
    subgraph KongCloud["Kong Konnect (cloud)"]
        Proxy["Proxy URL"]
        Auth["Validate API key"]
        Fwd["Forward to Service URL"]
        Cert["HTTPS + client cert<br/>kong-client.crt/key"]
        Proxy --> Auth
        Auth -->|"Valid"| Fwd
        Fwd --> Cert
    end

    subgraph YourInfra["Your infrastructure (VPC / servers)"]
        TLS["TLS: server.crt<br/>require client cert"]
        Verify["Verify client cert<br/>with ca.crt"]
        MTLS["mTLS proxy<br/>e.g. link.docnexus.ai:443"]
        App["App<br/>e.g. :8080 internal"]
        TLS --> Verify
        Verify -->|"Valid"| MTLS
        MTLS --> App
    end
    ...
    Req --> Proxy
    Cert -->|"Direct HTTPS"| TLS
    App --> Response["Response"]
    Response --> Client
```

**Step-by-step (production, mTLS enabled):**

```mermaid
flowchart TB
    subgraph Steps["Production request path with mTLS (no ngrok)"]
        S1["1. Client → Kong Proxy URL (e.g. https://xxx.kongcloud.dev/docnexus-link/...) with apikey"]
        S2["2. Kong Data Plane (cloud) validates API key → 401 if invalid"]
        S3["3. Kong looks up Service: URL = https://link.docnexus.ai (your production URL)"]
        S4["4. Kong attaches client certificate to outbound TLS"]
        S5["5. Kong opens TLS connection directly to your mTLS proxy (your VPC / load balancer)"]
        S6["6. Your mTLS proxy receives TLS handshake with client cert"]
        S7["7. Proxy verifies client cert with CA (ca.crt)"]
        S8["8. If cert invalid or missing → 403 or close connection"]
        S9["9. If valid → proxy forwards HTTP to app (internal :8080 or upstream)"]
...
**Why no ngrok in production:** Kong Konnect’s Data Plane runs in the cloud. Your backends (docnexus-link, etc.) run in **your** VPC or on your servers with **real hostnames** (e.g. `https://link.docnexus.ai`). Kong connects **directly** to that URL over the internet or private link. Ngrok is only for **local dev**, when the Data Plane cannot reach `localhost`—you expose your machine with ngrok so Kong can hit it. In production, both sides are reachable; no tunnel.

**What lives where (production, mTLS enabled):**

| Component | Role |
|-----------|------|
| **Kong (Control Plane)** | Stores **kong-client.crt** + **kong-client.key** as a Certificate; attached to the Service. |
| **Kong Data Plane (cloud)** | Validates API key, then connects **directly** to Service URL (e.g. `https://link.docnexus.ai`) over HTTPS and presents the client cert. |
| **Your mTLS proxy (your infra)** | Listens on HTTPS (e.g. :443 behind LB); **server.crt** + **server.key**; verifies client cert with **ca.crt**. Rejects (403) if no cert or invalid. Proxies to app over HTTP (internal). |
| **App** | HTTP only (e.g. :8080); only reachable from your mTLS proxy. |

Result: **only Kong** (with the client cert) can successfully call your mTLS proxy in production; everyone else gets 403 or handshake failure.

---

## Local / testing flow (Kong + ngrok + optional mTLS)

*Below is **local development only**: backends on localhost. **Ngrok is used only here**—because Kong Konnect’s Data Plane runs in the cloud and cannot reach your laptop, you run ngrok to expose localhost. **In production, Kong and your backends are both reachable (VPC or internet); no ngrok.***
...
| **Kong → Backend** | **Direct**: Kong connects straight to your service URL (e.g. `https://link.docnexus.ai`). **No ngrok.** | **Via ngrok**: Kong (cloud) can’t reach localhost, so you expose it with ngrok. Ngrok is **only for local testing**. |
| **Backends** | Deployed (e.g. ECS, K8s); real URLs | `localhost:8080`, `:3000`, `:8080`; ngrok tunnels to them for Konnect |
| **mTLS** | Kong → **direct HTTPS + client cert** → your mTLS proxy / backend | Optional: Kong → ngrok TCP → mTLS proxy :8443 → app |
| **Key management** | Same: docnexus-frontend → docnexus-api-platform → Kong Konnect API | Same |
| **Frontend** | docnexus-frontend deployed | Same app on localhost; env for API platform URL |
